...
if ($_REQUEST["from"]<>'' and $_REQUEST["to"]<>'') {
	...
	$order=" ORDER BY employees.rank asc, employees.employee_ID asc";
		$sql = "SELECT employees.employee_ID, employees.TitleOfCourtesy, employees.FirstName, employees.Middle_name, employees.LastName, employees.Position, employees.PhotoPath, department.dpt_name, section.section_name ".$add_years." FROM employees INNER JOIN department ON employees.department=department.id INNER JOIN section ON employees.section=section.id WHERE employees.apt_status='1'".$search_string.$search_city.$search_section.$search_rank.$order;
}

$sql_result = mysqli_query ($conn ,$sql ) or die ('request "Could not execute SQL query" '.$sql);
$profile = "active";
$terminated_profile = "active";
?>
