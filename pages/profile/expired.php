...
if ($_REQUEST["from"]<>'' and $_REQUEST["to"]<>'') {
	...
	$sql = "SELECT employees.employee_ID, employees.SSID, employees.TitleOfCourtesy, employees.FirstName, employees.Middle_name, employees.LastName, employees.Position, employees.HireDate, department.dpt_name, contracts.starting_date, contracts.due_date, employee_types.types, section.section_name ".$add_years." FROM employees INNER JOIN department ON employees.department=department.id INNER JOIN section ON employees.section=section.id LEFT OUTER JOIN `contracts` ON employees.employee_ID=contracts.employee_ID LEFT OUTER JOIN `employee_types` ON employees.rank=employee_types.id WHERE Year(DATE_ADD(`BirthDate`,INTERVAL 60 YEAR))='".mysqli_real_escape_string($conn, $_REQUEST["retire_yr"])."' AND employees.apt_status is null".$search_string.$search_city.$search_section.$search_rank." GROUP BY employees.employee_ID".$order;
		...
		$sql = "SELECT e.employee_ID, e.TitleOfCourtesy, e.FirstName, e.Middle_name, e.LastName, e.Position, e.PhotoPath, t.types, d.dpt_name, s.section_name, YEAR(e.`apt_statusDate`)-YEAR(e.HireDate) AS no_of_years, ABS(month(e.`apt_statusDate`) - month(e.`HireDate`)) as month, ABS(day(e.`apt_statusDate`) - day(e.`HireDate`)) as day, a.* FROM contracts a JOIN (SELECT `employee_ID`, MAX(`due_date`) due_date FROM contracts GROUP BY `employee_ID`) b ON a.`employee_ID`=b.`employee_ID` and a.`due_date`=b.`due_date` INNER JOIN employees e ON a.`employee_ID`=e.`employee_ID` INNER JOIN department d ON e.department=d.id INNER JOIN section s ON e.section=s.id INNER JOIN `employee_types` t ON e.rank=t.id WHERE a.`due_date` <= now()".$search_string.$search_city.$search_section.$search_rank;
}

$sql_result = mysqli_query ($conn ,$sql ) or die ('request "Could not execute SQL query" '.$sql);
$profile = "active";
$expired_contract_profile = "active";
?>
...
$rowsec = mysqli_fetch_array($sql_resultsec, MYSQL_ASSOC); ?>Employees with Expired Contracts in <?php echo $rowsec["section_name"];}else{ ?>Employees with Expired Contracts<?php }?></title>
           ...
           <!-- /.box -->
              ...
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Expired Contracts List in Profile View</h3><br>
