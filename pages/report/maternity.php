     ...
     <?php include("../../menu.php"); ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Maternity Leave Report for Permanent Employees
          </h1>
          <ol class="breadcrumb">
...
<?php while ($row = mysqli_fetch_array($sql_result)) {?>
  ...
  <tr data-href="../../profile_detail.php?id=<?php echo $row["employee_ID"]; ?>" class="empp">
   ...
   <td><?php $date = new DateTime($row["starting_date"]);
	$date->add(new DateInterval('P'.($row["num_used"]-1).'D'));
	$format = "jS F\, Y ";
	echo $date->format($format); ?></td>
    <td><?php $date = new DateTime($row["starting_date"]);
	$date->add(new DateInterval('P'.($row["num_used"]).'D'));
	$format = "jS F\, Y ";
	echo $date->format($format); ?></td>	
  </tr>
<?php
