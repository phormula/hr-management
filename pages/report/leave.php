...
if ($_REQUEST["from"]<>'' and $_REQUEST["to"]<>'') {
	...
	$order=" ORDER BY employees.rank asc, employees.employee_ID asc";
		$sql = "SELECT employees.employee_ID, employees.TitleOfCourtesy, employees.FirstName, employees.Middle_name, employees.LastName, employee_types.types, YEAR(NOW())-YEAR(employees.HireDate) AS no_of_years, employees.rank, SUM(employee_leave.`num_used`) AS USED FROM employees INNER JOIN employee_types ON employees.rank=employee_types.id LEFT OUTER JOIN `employee_leave` ON employees.employee_ID=employee_leave.employee_ID AND employee_leave.year=YEAR(NOW()) AND employee_leave.leave_type<>3 AND employee_leave.leave_type<>4 WHERE employees.apt_status is null AND employees.rank<>4 ".$search_string.$search_city.$search_section.$search_rank.$date." GROUP BY employees.employee_ID";
}

$sql_result = mysqli_query ($conn ,$sql ) or die ('request "Could not execute SQL query" '.$sql);
$report = "active";
$annual_report = "active";
?>
     ...
     <?php include("../../menu.php"); ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Leave Report for Permanent Employees
          </h1>
          <ol class="breadcrumb">
...
<?php while ($row = mysqli_fetch_array($sql_result)) {?>
  ...
  <tr data-href="../../profile_detail.php?id=<?php echo $row["employee_ID"]; ?>" class="empp">
    ...
    <td><?php
		$sqls = "SELECT YEAR(NOW())-YEAR(HireDate) AS no_of_years, rank FROM `employees` where `employee_ID`='".$emp_id."'";
	$sql_results = mysqli_query ($conn ,$sqls ) or die (die(json_encode(array('type'=>'error', 'text' =>'request "Could not execute SQL query" '.$sqls.''))));
	$rows = mysqli_fetch_assoc($sql_results);
	
	$sql2s = "SELECT * FROM `entitle_leave_days`";
	$sql_result2s = mysqli_query ($conn ,$sql2s ) or die (die(json_encode(array('type'=>'error', 'text' =>'request "Could not execute SQL query" '.$sql2s.''))));
	$row2s = mysqli_fetch_assoc($sql_result2s);
	
		if($row["no_of_years"]>=1 and $row["no_of_years"]<5 and $row["rank"]==2){
		$entitle = $row2s["one_to_five_years"];}
		else if($row["no_of_years"]>=5 and $row["no_of_years"]<10 and $row["rank"]==2){
		$entitle = $row2s["btn_five_and_ten"];}
		else if($row["rank"]==1){$entitle = $row2s["senior"];}
		else if($row["rank"]==3){$entitle = $row2s["contract"];}
		else{$entitle = $row2s["above_ten_years"];}
 		
		echo $entitle; ?></td>
   <td><?php if(empty($row["USED"])){echo "0";}else{ echo $row["USED"];} ?></td>
   <td><?php if(($entitle - $row["USED"])==0){echo '<b style="color:#F00">'.($entitle - $row["USED"]).'</b>';}else echo ($entitle - $row["USED"]);?></td>	
  </tr>
<?php
}
?>
