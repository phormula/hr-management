	...
	if (!IsInternetExplorer8OrLower) // images and drawings from canvas are supported on IE version 9 and higher, other modern browsers should work fine
{
    ...
    pdf.imageLoadFromUrl('picca1.png');
    // place this mage at given X, Y coordinates on the page
    pdf.imagePlace(250, 230);

} 
pdf.fontSetSize(24);

    // set font style with parameters: bold, italic, underline
	pdf.textSetBox(50, 310, 500, 500);
    pdf.fontSetStyle(true, false, false);
    pdf.textAddToBox('HR Report', false);
	pdf.textSetBox(50, 350, 500, 500);
	pdf.fontSetSize(20);
	pdf.fontSetStyle(false, true, false);
	pdf.textAddToBox('<?php echo date('F, Y');?>', false);

	pdf.pageAdd();
	pdf.textSetBoxPadding(3);

    // set text box
    pdf.textSetBox(50, 100, 500, 500);
	pdf.textSetAlign(BytescoutPDF.CENTER);
	pdf.fontSetStyle(true, false, true);
	pdf.textAddToBox("Content",true);
	
	pdf.textSetBox(50, 150, 500, 500);
	pdf.fontSetSize(12);
	pdf.textSetAlign(BytescoutPDF.LEFT);
	pdf.fontSetStyle(false, false, false);
	pdf.textAddToBox('1. Achievements', false);
	pdf.textAddToBox('2. HeadCount', false);
	pdf.textAddToBox('3. Recruitment', false);
	pdf.textAddToBox('4. Training', false);
	pdf.textAddToBox('5. Recognition', false);
	pdf.textAddToBox('6. HR Events', false);
	pdf.textAddToBox('7. Concern Areas', false);
	pdf.textAddToBox('8. Focus Areas in next Month', false);
	...
	pdf.textSetAlign(BytescoutPDF.LEFT);
	pdf.fontSetStyle(true, false, false);
	pdf.textAddToBox("Achievements",true);
	pdf.graphicsSetColor(0, 0, 0);

    // set line width
    pdf.graphicsSetLineWidth(1);
    // set line ending type
    pdf.graphicsSetLineEndType(BytescoutPDF.BUTT_CAP);

    // draw a line from one point to another
    pdf.graphicsDrawLine(50, 70, 540, 70);
	pdf.textAddToBox("allows to add bookmarks (outlines) tree; supports links (internal and external); supports text style, font size, color, style changes using  HTML formatting; 5 different fonts supported with bold, italic, underline styles and auto wrapping; ",true);
	pdf.textAddToBox(" ",true);	
	pdf.textAddToBox(" ",true);
	pdf.textAddToBox("Headcount",true);
	pdf.graphicsSetColor(0, 0, 0);
    ...
    // draw a line from one point to another
    pdf.graphicsDrawLine(50, 100, 540, 100);
           ...
           <!-- /.box -->
              ...
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Generate Monthly Report</h3>
                </div><!-- /.box-header -->
...
<div id="pdfreportimages" style="display: none;">
    <img src="picca1.png" id="pdfimage1"> 
    <img src="image2.jpg" id="pdfimage2"> 

</div>
<div class="col-xs-2"><div id="getpdf">
			</div></div>
			<script type="text/javascript">
				...
				function CreatePDFDownloadLink(PDFContentBase64) 
				{
					...
					if (CheckDataURISupport()) {
						...
						var buttonCode = '<button onclick=\"' + 'window.open(\'data:application\/pdf;base64,' + PDFContentBase64 + '\', \'_blank\');\" id=\"showPDFButton\" class=\"btn btn-default\">View Report</button>';
		...
		<div class="col-xs-10">	<div id="downloadify">
